...
import { LaserEyesLogo, useLaserEyes, WalletIcon } from '@omnisat/lasereyes-react'
import { Check, Copy, FileText, Pencil, Send, Upload } from 'lucide-react'
...
export function WalletConnectCard() {
  ...
  }, [connected, fetchData, resetState])
                      ...
                      inscriptions.slice(0, 4).map(insc => (
                        ...
                        >
                          ...
                          <div
                            ...
                            onClick={() =>
                              toggleInscriptionSelection(insc.inscriptionId || insc.id)
                            }
                          />
                  ...
                  {inscriptions.length > 0 && (
                    <>
                      ...
                      <Button
                        ...
                        disabled={
                          selectedInscriptionIds.length === 0 || !inscriptionRecipient || loading
                        }
                      >
                        <Send className="h-4 w-4" />
                      ...
                      {brc20Tokens.length > 0 ? (
                        brc20Tokens.map((token, i) => (
                          <SelectItem key={i} value={token.ticker}>
                            {token.ticker} (
                            {Number.parseFloat(token.overall || token.balance).toFixed(2)})
                          </SelectItem>
                        ))
                      ) : (
