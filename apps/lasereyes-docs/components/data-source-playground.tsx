...
const methods = [
  {
    ...
    ) => `// Get balance using ${dataSources.join(', ')}
...
console.log(\`Balance: \${balance} satoshis\`);`,
  },
  {
    ...
    code: (
      dataSources: DataSource[]
    ) => `// Get transaction details using ${dataSources.join(', ')}
...
console.log(tx);`,
  },
  {
    ...
    code: (
      dataSources: DataSource[]
    ) => `// Get inscriptions using ${dataSources.join(', ')}
...
console.log(\`Found \${inscriptions.length} inscriptions\`);`,
  },
  {
    ...
    code: (
      dataSources: DataSource[]
    ) => `// Get UTXOs using ${dataSources.join(', ')}
...
export function DataSourcePlayground({ className }: DataSourcePlaygroundProps) {
  ...
  const currentMethod =
    methods.find((m) => m.name === activeMethod) || methods[0]
        ...
        <div className="flex flex-col gap-6">
          ...
          >
            ...
            <div className="mt-4">
              ...
              <CodeBlock
                language="typescript"
                code={currentMethod.code(activeDataSources)}
                copyButton={true}
              />
            </div>
          </Tabs>
