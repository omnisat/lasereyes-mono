import {
  type LeatherProvider as LeatherRPC,
  RpcErrorCode,
...
import type { NetworkType, ProviderType, SignPsbtRequestParams } from '../../types'
import type {
  LaserEyesStoreType,
  SignMessageOptions,
...
export default class LeatherProvider extends WalletProvider {
  ...
  watchStateChange(
    ...
    handleStateChangePersistence(LEATHER, newState, changedKey, this.$valueStore)
  }

  initialize() {
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      this.observer = new window.MutationObserver(() => {
        const leatherLib = (window as unknown as { LeatherProvider: unknown })?.LeatherProvider
        if (leatherLib) {
  ...
  async connect(_: ProviderType): Promise<void> {
    ...
    const addresses = (
      await this.library.request('getAddresses', {
        network: this.config?.network ?? this.network,
      })
    ).result.addresses
  ...
  async switchNetwork(_network: NetworkType): Promise<void> {
    if (_network !== this.network) {
      ...
      const addresses = (await this.library.request('getAddresses', { network: _network })).result
        .addresses
  ...
  async requestAccounts(): Promise<string[]> {
    ...
    const response = await this.library?.request('getAddresses', {
      network: this.network,
    })
