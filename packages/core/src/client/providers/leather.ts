...
export default class LeatherProvider extends WalletProvider {
	...
	async connect(_: ProviderType): Promise<void> {
		...
		const taprootAddress = addresses.find((address) => address.symbol !== 'STX' && address.type === P2TR);
		const segwitAddress = addresses.find((address) => address.symbol !== 'STX' && address.type === P2WPKH);
	...
	async switchNetwork(_network: NetworkType): Promise<void> {
		if (_network !== this.network) {
			...
			const addresses = (
				await this.library.request("getAddresses", { network: _network })
			).result.addresses;
