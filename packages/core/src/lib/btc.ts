...
export const getAddressType = (address: string, network: NetworkType): string => {
  try {
...
export function getPublicKeyHash(address: string, network: NetworkType): Uint8Array {
  const btcNetwork = getBitcoinNetwork(network)
...
export async function waitForTransaction(txId: string, network: NetworkType): Promise<boolean> {
