...
import type { LasereyesUTXO } from '../../../types/utxo'
import { getPublicKeyHash } from '../../btc'
import { getBitcoinNetwork } from '../../helpers'
import { getSandshrewUrl, SANDSHREW_LASEREYES_KEY } from '../../urls'
...
export class SandshrewDataSource implements DataSource {
  ...
  async multicall(multiCall: unknown[]): Promise<RpcResponse[]> {
  ...
  async getOutputValueByVOutIndex(txId: string, vOut: number): Promise<number | null> {
  ...
  async getRecommendedFees(): Promise<{ fastFee: number; minFee: number }> {
    const response = (await this.call('esplora_fee-estimates', [])) as RpcResponse
  ...
  async getBalances(address: string | string[]): Promise<SandshrewBalancesResult[]> {
    if (Array.isArray(address)) {
      const multiCall = address.map(addr => ['sandshrew_balances', [{ address: addr }]])
      const result = await this.multicall(multiCall)
      return result.map(e => e.result) as SandshrewBalancesResult[]
    }

    const response = (await this.call('sandshrew_balances', [{ address }])) as RpcResponse
