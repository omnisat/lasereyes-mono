...
export function normalizeInscription(
  ...
  const id = extractStringValue(insc, ['id', 'inscription_id', 'inscriptionId'], '')

  // Extract inscription number
  const number = extractNumberValue(insc, ['num', 'number', 'inscriptionNumber'], 0)

  // Extract owner address
  const address = extractStringValue(insc, ['address', 'owner', 'ownerAddress'], '')
  ...
  if (output) {
    if (!txid || !vout) {
      ...
      txid = output.split(':')[0]
      vout = Number.parseInt(output.split(':')[1], 10)
    }
  }
