 ...
 */
export function normalizeInscription(
  insc: any,
  ...
  const number = extractNumberValue(
    insc,
  ...
  const address = extractStringValue(
    insc,
  ...
  const contentType = extractStringValue(
    insc,
    ['content_type', 'contentType', 'mime', 'mimeType'],
    'unknown'
  )

  let txid = extractStringValue(
    insc,
    ['utxo_txid', 'txid', 'transaction_id'],
    ''
  )
  ...
  const outputValue = extractNumberValue(
    insc,
    ['output_value', 'value', 'outputValue', 'postage', 'satoshis'],
    0
  )
  const height = extractNumberValue(insc, ['height', 'block_height'], 0)

  let genesisTransaction = extractStringValue(
    insc,
 ...
 */
export function normalizeBrc20Balances(
  data: any,
  source: string = 'unknown'
): Brc20Balance[] {
  ...
  if (data.data && typeof data.data === 'object') {
    ...
    Object.entries(data.data).forEach(
      ([ticker, balanceData]: [string, any]) => {
        const overall = extractStringValue(
          balanceData,
          ['overall', 'overallBalance', 'total', 'totalBalance'],
          '0'
        )
        const transferable = extractStringValue(
          balanceData,
          ['transferable', 'transferableBalance'],
          'na'
        )
        const available = extractStringValue(
          balanceData,
