...
import type { ChainDataSource, Client } from '@omnisat/lasereyes-client'
import { createClient, mergeDataSources } from '@omnisat/lasereyes-client'
 ...
 */
export interface LaserEyesCoreConfig<networksConfig extends Record<NetworkId, NetworkConfig>> {
  /** Default network ID */
  network?: NetworkId
  ...
  networks?: Partial<networksConfig>
}
...
export class LaserEyesCore {
  ...
  private createConnectorFromAnnouncement(announcement: WalletAnnouncement): Connector {
    ...
    const connector: Connector = {
      ...
      getNetworkId: () => adapter.request({ method: 'bitcoin_getNetwork' }) as Promise<NetworkId>,
