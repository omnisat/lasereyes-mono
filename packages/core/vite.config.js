...
export default defineConfig(({ mode }) => {
  // UMD target (`vite build --mode umd`): a single-file, script-tag drop-in
  // that attaches to `window.LaserEyes` and registers with AMD/CommonJS
  // loaders when present. It's a different shape from the multi-entry ESM
  // lib below — one entry (`src/umd.ts`), `umd` format, no externals
  // (drop-in users have no module resolver) — so it lives in this branch
  // rather than a separate config file. `emptyOutDir: false` keeps the ESM
  // `dist/` produced by the default build.
  if (mode === 'umd') {
    return {
      build: {
        emptyOutDir: false,
  ...
  // Default target: multi-entry ESM library.
  return {
    plugins: [
      dts(),
      process.env.ANALYZE
