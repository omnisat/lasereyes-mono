...
export const sendBrc20 = async ({
  ...
  try {
    ...
    const { fastestFee } = await ds.getRecommendedFees()

    const estimatedSize = 5 * 34 * 1
    const sendInscriptionSatsNeeded = Math.floor(estimatedSize * fastestFee * 1)

    // delay for 10 seconds
    await new Promise((resolve) => setTimeout(resolve, 10000))


    const utxosGathered: MempoolUtxo[] = await ds.getAddressUtxos(
      paymentAddress
    )
