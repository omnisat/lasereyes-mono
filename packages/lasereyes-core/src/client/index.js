...
import { LEATHER, MAGIC_EDEN, OKX, ORANGE, OYL, PHANTOM, UNISAT, WIZZ, XVERSE, } from '../constants/wallets';
...
export class LaserEyesClient {
    ...
    constructor(stores, config) {
        ...
        subscribeKeys(this.$store, ['isInitializing'], (v) => this.handleIsInitializingChanged(v.isInitializing));
    ...
    handleIsInitializingChanged(value) {
        if (!value) {
            const defaultWallet = localStorage?.getItem(LOCAL_STORAGE_DEFAULT_WALLET);
            if (defaultWallet) {
    ...
    async signMessage(message, toSignAddress) {
        ...
        if (this.$providerMap[this.$store.get().provider]) {
            try {
                return await this.$providerMap[this.$store.get().provider]?.signMessage(message, toSignAddress);
            }
            catch (error) {
                if (error instanceof Error) {
                    if (error.message.toLowerCase().includes('not implemented')) {
    ...
    async signPsbt(tx, finalize = false, broadcast = false) {
        ...
        if (this.$store.get().provider &&
            this.$providerMap[this.$store.get().provider]) {
            try {
                const signedPsbt = await this.$providerMap[this.$store.get().provider]?.signPsbt(tx, psbtHex, psbtBase64, finalize, broadcast);
                return signedPsbt;
            }
            catch (error) {
                if (error instanceof Error) {
