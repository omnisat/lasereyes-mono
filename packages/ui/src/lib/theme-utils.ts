...
export function applyThemeColors(
  ...
  Object.entries(colors).forEach(([key, value]) => {
    if (value) {
      const cssKey =
        customPropertyPrefix + key.replace(/([A-Z])/g, '-$1').toLowerCase()
...
export function setPrimaryColor(
  ...
  root.style.setProperty(
    customPropertyPrefix + 'primary',
    hslToCssValue(primary)
  )
  root.style.setProperty(
    customPropertyPrefix + 'primary-foreground',
    hslToCssValue(primaryForeground)
  )
}
 ...
 */
export function onSystemDarkModeChange(
